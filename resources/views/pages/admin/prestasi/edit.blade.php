...
@section('header-title', 'Edit Prestasi Santri')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <h2 class="font-headline font-extrabold text-3xl text-primary tracking-tight">Edit Prestasi Santri</h2>
        <p class="text-on-surface-variant text-sm mt-1">Ubah data prestasi hafalan kitab santri.</p>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.prestasi.update', $prestasi) }}" method="POST" enctype="multipart/form-data" class="bg-surface-container-lowest p-8 rounded-xl shadow-sm space-y-6">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select name="santri_id" required
                    ...
                    @foreach($santriList as $santri)
                        <option value="{{ $santri->id }}" {{ old('santri_id', $prestasi->santri_id) == $santri->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nama_kitab" required value="{{ old('nama_kitab', $prestasi->nama_kitab) }}"
                           class="w-full bg-surface-container-high border-none rounded-xl py-3 px-4 text-on-surface focus:bg-surface-container-highest focus:ring-0 transition-all"
                ...
                <div>
                    ...
                    <select name="kategori"
                        ...
                        <option value="hafalan" {{ old('kategori', $prestasi->kategori) == 'hafalan' ? 'selected' : '' }}>Hafalan</option>
                        <option value="tajwid" {{ old('kategori', $prestasi->kategori) == 'tajwid' ? 'selected' : '' }}>Tajwid</option>
                        <option value="qiraat" {{ old('kategori', $prestasi->kategori) == 'qiraat' ? 'selected' : '' }}>Qira'at</option>
                        <option value="hadits" {{ old('kategori', $prestasi->kategori) == 'hadits' ? 'selected' : '' }}>Hadits</option>
                        <option value="fiqih" {{ old('kategori', $prestasi->kategori) == 'fiqih' ? 'selected' : '' }}>Fiqih</option>
                        <option value="bahasa" {{ old('kategori', $prestasi->kategori) == 'bahasa' ? 'selected' : '' }}>Bahasa Arab</option>
                    </select>
                    @error('kategori')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="status" required
                            class="w-full bg-surface-container-high border-none rounded-xl py-3 px-4 text-on-surface focus:bg-surface-container-highest focus:ring-0 transition-all">
                        <option value="belum_dihafal" {{ old('status', $prestasi->status) == 'belum_dihafal' ? 'selected' : '' }}>Belum Dihafal</option>
                        <option value="sedang_dihafal" {{ old('status', $prestasi->status) == 'sedang_dihafal' ? 'selected' : '' }}>Sedang Dihafal</option>
                        <option value="telah_dihafalkan" {{ old('status', $prestasi->status) == 'telah_dihafalkan' ? 'selected' : '' }}>Telah Dihafalkan</option>
                    </select>
                    @error('status')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    @if($prestasi->foto_kitab)
                        <div class="mb-3">
                            <img src="{{ Storage::url($prestasi->foto_kitab) }}" alt="Foto Kitab" class="w-32 h-32 object-cover rounded-lg border-2 border-primary/20">
                            <p class="text-xs text-on-surface-variant mt-2">Foto saat ini. Upload foto baru untuk mengganti.</p>
                        </div>
                    @endif
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="nilai" value="{{ old('nilai', $prestasi->nilai) }}"
                           class="w-full bg-surface-container-high border-none rounded-xl py-3 px-4 text-on-surface focus:bg-surface-container-highest focus:ring-0 transition-all"
                ...
                <div>
                    ...
                    <input type="number" name="skor" min="0" max="100" value="{{ old('skor', $prestasi->skor) }}"
                           class="w-full bg-surface-container-high border-none rounded-xl py-3 px-4 text-on-surface focus:bg-surface-container-highest focus:ring-0 transition-all"
                           placeholder="0-100">
                    @error('skor')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" name="poin" min="0" value="{{ old('poin', $prestasi->poin) }}"
                           class="w-full bg-surface-container-high border-none rounded-xl py-3 px-4 text-on-surface focus:bg-surface-container-highest focus:ring-0 transition-all"
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="bulan_tahun_selesai" value="{{ old('bulan_tahun_selesai', $prestasi->bulan_tahun_selesai) }}"
                           class="w-full bg-surface-container-high border-none rounded-xl py-3 px-4 text-on-surface focus:bg-surface-container-highest focus:ring-0 transition-all"
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="ustadz_pembimbing" value="{{ old('ustadz_pembimbing', $prestasi->ustadz_pembimbing) }}"
                           class="w-full bg-surface-container-high border-none rounded-xl py-3 px-4 text-on-surface focus:bg-surface-container-highest focus:ring-0 transition-all"
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="keterangan" rows="2"
                              ...
                              placeholder="Keterangan tambahan tentang prestasi...">{{ old('keterangan', $prestasi->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <textarea name="catatan_ustadz" rows="3"
                              class="w-full bg-surface-container-high border-none rounded-xl py-3 px-4 text-on-surface focus:bg-surface-container-highest focus:ring-0 transition-all resize-none"
                              placeholder="Catatan atau feedback dari ustadz...">{{ old('catatan_ustadz', $prestasi->catatan_ustadz) }}</textarea>
                ...
                <div>
                    ...
                    <input type="text" name="tags" value="{{ old('tags', $prestasi->tags) }}"
                           class="w-full bg-surface-container-high border-none rounded-xl py-3 px-4 text-on-surface focus:bg-surface-container-highest focus:ring-0 transition-all"
