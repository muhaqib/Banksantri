    ...
    <form action="{{ route($routePrefix.'.update', $blog) }}" method="POST" enctype="multipart/form-data" class="bg-surface-container-lowest p-5 sm:p-6 rounded-xl shadow-sm space-y-6">
        ...
        <div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    @include('pages.admin.blog.partials.content-editor', ['content' => old('content', $blog->content)])
                    @error('content')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div>
            ...
            <div>
                ...
                <p class="text-xs text-on-surface-variant mt-2">Jika dicentang, blog akan dipublikasikan. Jika tidak, blog akan disimpan sebagai draft.</p>
            </div>
        </div>
