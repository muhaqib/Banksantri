...
<div class="space-y-6">
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
            <p class="text-sm font-bold text-primary">Akademik Semester</p>
            <h1 class="font-headline text-3xl font-black">Input Nilai Tarbiyah</h1>
            <p class="text-sm text-on-surface-variant">Input manual atau import Excel berdasarkan NIS santri.</p>
        </div>
    </div>
    ...
    <div class="grid gap-4 lg:grid-cols-3">
        <form method="POST" action="{{ route('petugas.tarbiyah.import') }}" enctype="multipart/form-data" class="rounded-xl bg-surface-container-lowest p-4 shadow-sm lg:col-span-2">
            ...
            <p class="font-bold text-on-surface">Import Excel</p>
    ...
    <form method="POST" action="{{ route('petugas.tarbiyah.store') }}" class="overflow-hidden rounded-xl bg-surface-container-lowest shadow-sm">
        ...
        <div class="flex flex-col gap-3 border-t border-surface-container p-4 md:flex-row md:items-center md:justify-between">
            ...
            <button class="btn-primary justify-center"><span class="material-symbols-outlined">save</span> Simpan Nilai</button>
        </div>
    </form>
</div>
@endsection
