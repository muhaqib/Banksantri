@extends('components.sidebar', ['activeRole' => 'santri'])
...
<div class="max-w-4xl mx-auto" x-data="{ activeTab: 'email' }">
    ...
    <div class="bg-surface-container-low rounded-2xl shadow-sm mb-6">
        ...
        <div x-show="activeTab === 'email'" class="p-6">
            <form action="{{ route('santri.profile.email') }}" method="POST">
                @csrf
                <div class="space-y-6">
                    <!-- Current Email -->
                    <div>
        ...
        <div x-show="activeTab === 'password'" class="p-6">
            <form action="{{ route('santri.profile.password') }}" method="POST">
                @csrf
                <div class="space-y-6">
                    <!-- Current Password -->
                    <div>
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-center justify-between py-3 border-b border-outline-variant/10">
                <div>
                    ...
                    <p class="font-medium text-on-surface">{{ auth()->user()->email }}</p>
                </div>
            </div>
            <div class="flex items-center justify-between py-3">
                <div>
                    <p class="text-sm text-on-surface-variant">NIS</p>
                    <p class="font-medium text-on-surface">{{ auth()->user()->nis ?? '-' }}</p>
                </div>
            </div>
        </div>
    </div>
